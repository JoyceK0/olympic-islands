...
import com.github.JoyceK0.Olympic_Islands.component.Animation2D;
import com.github.JoyceK0.Olympic_Islands.component.Facing;
...
public class AnimationSystem extends IteratingSystem {
    private static final float FRAME_DURATION = 1 / 8f; // default 8 frames per second
    ...
    protected void processEntity(Entity entity, float deltaTime) {
        ...
        if(animation2D.isDirty() || facingDirection != animation2D.getDirection()){ // dirty being true means animation must be recalculated. if facing direction is different from direction of animation, animation must be recalculated.
            updateAnimation(animation2D, facingDirection); // updates animation
            stateTime = 0f;
        } else {
            stateTime = animation2D.incAndGetStateTime(deltaTime); // else, just updates stateTime
        }
